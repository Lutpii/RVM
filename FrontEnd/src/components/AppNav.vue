...
/* Active tab's icon pops up into a small circle — no colored strip behind
   it (tried and dropped per feedback), just the icon lifting and taking on
   the nav's own background color so it reads as "notched" on its own.
   Mobile only — see the ≥769px override below. */
...
@media (min-width: 769px) {
  ...
  /* The lift-into-a-circle notch is a mobile bottom-nav pattern; on the
     desktop row layout it just falls back to the original color-only
     active state, unchanged from before this feature existed. */
  .nav-item-active .nav-icon {
